...
describe('provider settings', () => {
  it('returns an empty provider map when storage has no settings', async () => {
    await expect(loadSettings(createStorage())).resolves.toEqual({ providers: {} });
  });

  it('ignores malformed stored settings', async () => {
    await expect(loadSettings(createStorage({ arkSettings: 'broken' }))).resolves.toEqual({
      providers: {},
    });
  });
  ...
  it('keeps each provider configuration and activates the last saved one', async () => {
    ...
    await saveProvider(storage, 'openai', { apiKey: 'openai-key', model: 'gpt-4.1' });
    await saveProvider(storage, 'anthropic', { apiKey: 'anthropic-key', model: 'claude-sonnet-4' });
