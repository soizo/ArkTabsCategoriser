...
function modelIds(value: unknown): string[] {
  ...
  return uniqueModels(value.models.flatMap((item) => {
...
export function createGeminiProvider(fetchImpl: Fetch): Provider {
  return {
    ...
    async categorise(settings, tabs, locale, signal) {
      ...
      return parseCategorisation(candidateText(value), tabs.map(({ id }) => id));
    },
  };
}
