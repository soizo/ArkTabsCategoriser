...
export function createAnthropicProvider(fetchImpl: Fetch): Provider {
  return {
    async listModels(settings, signal) {
      const value = await requestJson(fetchImpl, `${BASE_URL}/models`, {
        method: 'GET',
    ...
    async categorise(settings: ProviderSettings, tabs: TabInput[], locale, signal) {
      const prompt = buildCategorisationPrompt(tabs, locale);
      const value = await requestJson(fetchImpl, `${BASE_URL}/messages`, {
        method: 'POST',
        headers: headers(settings),
        body: JSON.stringify({
      ...
      return parseCategorisation(messageText(value), tabs.map(({ id }) => id));
    },
  };
}
